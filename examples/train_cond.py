...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, target_path, tokenizer, seq_len, encoding="utf-8", on_memory=True, keep_answerable=True):
    ...
    def __getitem__(self, item):
        ...
        tokens_b = [self.tokenizer.tokenize(t) for t in t2]
        buff_size = sum([len(t) for t in tokens_b]) + len(tokens_b) - 1
    ...
    def get_corpus_line(self, item):
        ...
        if self.on_memory:
            ...
            first_targets = [words for tag, words in targets if "-"  not in tag  and "W" not in tag]
            middle_targets_int = [(tag, words) for tag, words in targets if "-"   in tag]
            middle_targets = []
            for i in range(20):
                for tag, words in middle_targets_int:
                    if str(i) in tag:
                        middle_targets.append(words)

            second_targets = [words for tag, words in targets if "W" in tag]
...
def random_word(tokens, tokenizer, question=False, tokens_question=None, tokens_target=None):
    ...
    if not question:
        ...
        for i, t in enumerate(tokens):
            if i  == len(tokens) - 1:
                output_label += tokenizer.convert_tokens_to_ids(t)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                    if args.test_run and global_step == 2:
