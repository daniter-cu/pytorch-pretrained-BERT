...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, target_path, tokenizer, seq_len, encoding="utf-8", on_memory=True,
        ...
        self.examples = []
        self.raw_targets = {}
        self.multihint = multi_hint

        # load samples into memory
        if on_memory:
            for i in range(24):
                with open(target_path%str(i), "rb") as handle:
                    self.raw_targets.update(pickle.load(handle))
    ...
    def __getitem__(self, item):
        ...
        t1, t2, is_next_label = self.get_example(item)
        ...
        targets = []
        '''
        t2 is a list of (tag, words) tuples.
        If self.multihint == True we want to include the type hint on each entry
        if it is not, then we only want to include the hint on the last one.
        span is therefore either just the entry to be added to the input,
        or it is the tag appended to the entry.
        Some tags have extra markers for order ("-1", "-2", etc) which will be removed
        for clean_tag
        '''
        for tidx, (tag, words) in enumerate(t2):
            clean_tag = tag[1:-1]
            if "-" in tag:
                clean_tag = tag[:tag.index("-")]
            if self.multihint or (tidx == len(t2) - 1):
                span = clean_tag + " [SEP] " + words
            else:
                span = words
            if tidx == len(t2) - 1:
                tokens_b.append(self.tokenizer.tokenize(clean_tag))
                targets = self.tokenizer.tokenize(words)
            else:
                tokens_b.append(self.tokenizer.tokenize(span))
        assert len(targets) > 0, "No targets found."

        buff_size = sum([len(t) for t in tokens_b]) + len(tokens_b) - 1 + len(targets)
        ...
        cur_example = InputExample(guid=cur_id, tokens_a=tokens_a, tokens_b=tokens_b, is_next=is_next_label, targets=targets)
    ...
    def get_corpus_line(self, item):
        ...
        if self.on_memory:
            ...
            second_targets = [(tag, words) for tag, words in targets if "W" in tag]
            random.shuffle(first_targets)
            random.shuffle(second_targets)

            targets = (first_targets + middle_targets + second_targets)[:target_i+1]
            t2 = targets
...
def convert_example_to_features(example, max_seq_length, tokenizer):
    ...
    lm_label_ids += tokenizer.convert_tokens_to_ids(targets)
...
def main():
    ...
    parser.add_argument("--question_parts_file",
                        default=None,
                        type=str,
                        required=True,
                        help="The question parts file.")
    ...
    parser.add_argument("--multihint",
                        action='store_true',
                        help="Whether to add hint all the time.")
    ...
    if args.do_train:
        ...
        train_dataset = BERTDataset(args.train_file, args.question_parts_file, tokenizer, seq_len=args.max_seq_length,
                                     on_memory=args.on_memory, multi_hint=args.multihint)
        val_dataset_ans = BERTDataset(args.dev_file, args.question_parts_file, tokenizer, seq_len=args.max_seq_length,
                                    on_memory=args.on_memory, multi_hint=args.multihint)
        val_dataset_unans = BERTDataset(args.dev_file, args.question_parts_file, tokenizer, seq_len=args.max_seq_length,
                                    on_memory=args.on_memory, keep_answerable=False, multi_hint=args.multihint)
        num_train_steps = int(
